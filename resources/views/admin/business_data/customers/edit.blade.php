...
<div class="admin-page">
    ...
    <div class="admin-panel">
        <div class="admin-panel-body">
            <form method="POST" action="{{ route('admin.business_data.customers.update', $customer->id) }}">
                @csrf
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <select name="business_id" class="form-select" required>
                            @foreach($businesses as $b)
                                <option value="{{ $b->id }}" @selected(old('business_id', $customer->business_id) == $b->id)>{{ $b->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="group_id" class="form-select">
                            ...
                            @foreach($groups as $g)
                                <option value="{{ $g->id }}" @selected(old('group_id', $customer->group_id) == $g->id)>{{ $g->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-12">
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="is_blacklisted" id="is_blacklisted" @checked(old('is_blacklisted', $customer->is_blacklisted))>
